...
{ "module_build", "module_coreupgrade", "module_shieldgen",
	...
	"module_m_hyperspace_scanner", "module_m_cloak", "module_defense_2", "module_defense_3", "module_m_cloakdisruptor", "module_m_brewery", "module_o_refinery", 0};
		...
		for (uint i = 0; i < base->modules.size(); i++)
		{
			if (base->modules[i] == this)
			{
				...
				case Module::TYPE_M_CLOAKDISRUPTOR:
					...
					break;
				case Module::TYPE_M_BREWERY:
					base->modules[i] = new FactoryModule(base, Module::TYPE_M_BREWERY);
					break;
